...
nhgis_tr_2000 <- vroom::vroom(
  here::here("data", "preproc", "tract", "nhgis_tr_2000_agg.csv")
) %>%
  rename(POP_TRA = POP) %>%
...
# If using block xwalk geog reallocated data:

# 2000 MRSF with 2010 tract boundaries
# mrsf <- vroom::vroom(
#   here::here("data", "preproc", "mrsf", "mrsf_2000_agg_2010cty.csv")
# )
# 
# nhgis_cty_2000 <-  vroom::vroom(
#   here::here("data", "preproc", "county", "nhgis_cty_2000_2010_agg_standardized.csv")
# ) %>%
#   filter(DATAYEAR == 2000, STATEA != "72") %>%
#   rename(POP_CTY = POP) %>%
#   select(-STATE, -COUNTY)

# Calculate cty pop ratios for new race groups from MRSF
mrsf_comb <- full_join(mrsf, nhgis_cty_2000) %>%
...
write_csv(mrsf_realloc, here::here("data", "preproc", "tract", "tract_mrsf_reallocation_2000.csv"))
