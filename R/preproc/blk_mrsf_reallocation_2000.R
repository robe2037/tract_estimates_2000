...
blk_reallocate <- function(dec_data, mrsf_data, var_key) {
...
check_reallocation <- function(realloc_data) {
  ...
  check_realloc <- realloc_data %>%
    ...
    summarize(tot = round(sum(POP), 10) == round(sum(POP_SUB_NEW, na.rm = TRUE), 10)) %>%
    pull(tot) %>%
    all()
...
blk_convert_geog_batch <- function(extract_numbers) {
      ...
      blk_geog_converted <- purrr::imap_dfr(
        ...
        ~{
          ...
          blk_data %>%
            ...
            blk_reallocate(mrsf_data, var_key) %>%
            check_reallocation() %>%
